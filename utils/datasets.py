...
import yaml
from PIL import ExifTags, Image, ImageOps, ImageGrab
...
class LoadScreen:
    ...
    def __init__(self, img_size=640, stride=32, auto=True):
        ...
        self.stride = stride
        self.files = ImageGrab.grab()  # 获取屏幕对象
    ...
    def __next__(self):
        ...
        self.count += 1
        # captureImage =  # 抓取屏幕
        img0 = cv2.cvtColor(np.array(ImageGrab.grab() ), cv2.COLOR_RGB2BGR)  # 彩色化：灰度图像转为彩色图像
        # img0 = cv2.imread(frame)  # BGR
        assert img0 is not None, f'Image Not Found {path}'
        s = f'image {self.count}/{self.nf} {path}: '
        # Padded resize 填充调整大小
